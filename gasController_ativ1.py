...
'''
Falta desenvolver a medida de desempenho:

Fazer cálculo de probabilidade e estimar o desempenho
...
for i in range (0, len(environment) - 1) :
    if (environment [ i ] == environment [ i + 1 ]):
        ...
        if (i == len (environment) - 2):
            print(environment [ i ], ',', desempenho + 1)
    else:
        print(environment [ i ], ',', rdesempenhoep + 1)
        desempenho = 0
